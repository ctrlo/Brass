...
{   my ($self, %params) = @_;
    ...
    if ($type eq 'pwd')
    {
        $self->run_pwd(%params, pwdpass => $self->pwdpass, local => 1);
    }
    elsif ($type eq 'cert')
    {
...
{   my ($self, %params) = @_;
    ...
    my $is_local = $params{local};
    my $user     = $params{user};

    $is_local || $user
        or panic "Local or user needed for run_pwd";

    Brass::Actions::is_allowed_action($action)
        or error __x"Invalid action: {action}", action => $action;

    my $username = $params{param};

    !Brass::Actions::action_requires_pwd($action) || $username
    ...
    my $server_rs = $self->schema->resultset('Server')->search({ name => $server })->next
        or error __x"Server {name} not found", name => $server;

    $is_local || grep $user->has_servertype($_->servertype_id), $server_rs->server_servertypes
        or error __"You do not have access to this server";

    my $pw_rs = $server_rs->pws->search({
    ...
    if (defined $pass)
    {
        ...
        {
            error __x"Please use a secure password, provided password is not secure: {errs}",
                errs => join(',', @{ $pwcheck->error_list });
        }
    }
    ...
    if (defined $pass)
    {
        ...
        $pass = $cipher->decrypt($pw_rs->pwencrypt);
    }
